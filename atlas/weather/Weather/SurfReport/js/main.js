;(function(window) {
	...
	console.log($.urlParam('city_name'),'+++');
	$.get("https://query.yahooapis.com/v1/public/yql?q=select * from weather.forecast where woeid in (select woeid from geo.places(2) where text='"+$.urlParam('city_name')+"')&format=json", function(data, status){
	       var weather_data =  data.query.results.channel;
	       if (weather_data.length > 1) {
	       	weather_data = data.query.results.channel[0];
	       }
	       console.log('====',weather_data);
	       metaData = [
			{
				'name' : weather_data.location.city+", "+weather_data.location.country,
				'weather' : []
			}
		];
		console.log(metaData[0].weather,'))))');
		var counter =0;
		weather_data.item.forecast = weather_data.item.forecast.slice(0,6);
		weather_data.item.forecast.forEach(function(each_day){
			counter++;
			var insert = { // today!
						'state': { 'display' : 1, 'hourly' : [6,6,2,1,1,7]},
						'day' : each_day.day,
						'date' : each_day.date,
						'air' : { 'display' : (parseInt(each_day.high) + parseInt(each_day.low))/2, 'hourly' : []},
						'windspeed' : { 'display' : weather_data.wind.speed, 'hourly' : []},
						...
						'water' : { 'display' : 0, 'hourly' : [0]}
					};
			switch (each_day.text){
				case "Sunny":
					insert.state.display = 1;
					break;
				case "Partly Cloudy":
					insert.state.display = 2;
					break;
				case "Mostly Cloudy":
					insert.state.display = 3;
					break;
				case "Snow Showers":
					insert.state.display = 4;
					break;
				case "Rain":
					insert.state.display = 4;
					break;
		...
		metaData[0].weather.push(insert);
		})
	});
$(document).ajaxStop(function(){
	...
	DayForecast.prototype._buildHourlyLayout = function() {
		...
		function init() {
			...
			createGraph();
			initEvents();
			$('#city_name_header').text(metaData[0].name)
		}
